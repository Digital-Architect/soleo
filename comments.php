...
<div id="comments">
	...
	?>
		...
		<p>
			<?php echo __('Comments are closed', 'wp_soleo')?>
		</p>
	...
	$comments_arg = array(
		...
		'fields' => apply_filters( 'comment_form_default_fields', array(
				'autor' 				=> '<div class="mb-3">' . '<label for="author">' . __( 'Name', 'wp_soleo' ) . '</label> ' . ( $req ? '<span>*</span>' : '' ) .
										'<input id="author" name="author" class="form-control" type="text" value="" size="30"' . $aria_req . ' />'.
										'<p id="d1" class="text-danger"></p>' . '</div>',
				'email'					=> '<div class="mb-3">' .'<label for="email">' . __( 'Email', 'wp_soleo' ) . '</label> ' . ( $req ? '<span>*</span>' : '' ) .
										'<input id="email" name="email" class="form-control" type="text" value="" size="30"' . $aria_req . ' />'.
										'<p id="d2" class="text-danger"></p>' . '</div>',
				'url'					=> '')),
				'comment_field'			=> '<div class="mb-3">' . '<label for="comment">' . __( 'Comment', 'wp_soleo' ) . '</label><span>*</span>' .
	...
	?>
		...
		<script>
			...
			*/
			...
			if (x === null || x === "") {
				d1.innerHTML = "<?php echo __('Name is required', 'wp_soleo'); ?>";
				flag = false;
			} else {
				d1.innerHTML = "";
			}
			...
			if (y === null || y === "") {
				d2.innerHTML = "<?php echo __('Email is required', 'wp_soleo'); ?>";
				flag = false;
			} else {
				d2.innerHTML = "";
			}
			...
			if (z === null || z === "") {
				d3.innerHTML = "<?php echo __('Comment is required', 'wp_soleo'); ?>";
				flag = false;
			} else {
				d3.innerHTML = "";
			}
